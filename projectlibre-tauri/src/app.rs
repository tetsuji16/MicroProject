...
fn draw_inazuma_polyline(painter: &egui::Painter, start: Pos2, end: Pos2, stroke: Stroke) {
    ...
    let bend = (dx.abs() * 0.28).clamp(10.0, 32.0);
    let vertical = (dy.abs() * 0.25).clamp(4.0, 10.0);
    let direction = if dx >= 0.0 { 1.0 } else { -1.0 };
    let mid_x = start.x + dx * 0.5;
    let path = [
        start,
        Pos2::new(start.x + bend * 0.45 * direction, start.y - vertical),
        Pos2::new(mid_x - bend * 0.25 * direction, start.y + vertical),
        Pos2::new(mid_x + bend * 0.25 * direction, end.y - vertical),
        Pos2::new(end.x - bend * 0.45 * direction, end.y + vertical),
        end,
    ];
